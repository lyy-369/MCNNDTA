...
class MCNN_FIFTHGAT_GCN(torch.nn.Module):
    def __init__(self,block_num=3, vocab_protein_size=26, n_output=1, num_features_xd=78, num_features_xt=25,
                 n_filters=32, embed_dim=128, output_dim=128, dropout=0.2, embedding_size=128, filter_num=32, out_dim=1):

        super(MCNN_FIFTHGAT_GCN, self).__init__()

        self.n_output = n_output
        self.conv1 = GATConv(num_features_xd, num_features_xd, heads=10)
        self.conv2 = GCNConv(num_features_xd*10, num_features_xd*10)
        self.conv3 = GCNConv(num_features_xd*10, num_features_xd*10)
        self.conv4 = GCNConv(num_features_xd*10, num_features_xd*10)
        self.conv5 = GCNConv(num_features_xd*10, num_features_xd*10)
        self.conv6 = GCNConv(num_features_xd*10, num_features_xd*10)

        self.fc_g1 = torch.nn.Linear(num_features_xd*10*2, 1500)#?512是怎么来的
